...
public class UserDaoImpl implements UserDao {
    ...
    private static String getGunja() {
        return "gunja";
    }
    ...
    public List<UserVO> getList() throws SQLException {
        ...
        //검색 행이 많은 경우에는 행 하나 당 vo가 하나가 필요
        //행이 여러개 이면 이 vo를 넣어줄 list가 필요!
        List<UserVO> list = new ArrayList<UserVO>();

        while (rs.next()){ //반복할 때 마다 다음 행을 가리키면서(cursor, 커서) 있는지 체크해줌
            //있으면, 각 컬럼값을 꺼내와서 vo(bag) 에 넣어주자.

            UserVO user = new UserVO("test01", "pass123", "Updated Name", "ADMIN"); //가방 만들어주기

            user.setId(rs.getString("id")); //id는 컬럼명
            user.setName(rs.getString("name"));
            //컬럼명 대신 resultset(table)에 있는 인덱스를 쓸 수 있다.
            //컬럼명을 더 권장!
        ...
        pstmt.close();
        rs.close();
        return list; //자바는 리턴할 때 무조건 하나로 묶어줘야함
    }
    ...
    public int update(UserVO user) throws SQLException {
        ...
        System.out.println("3단계 : sql문 객체로 만들기 성공함.");
        pstmt.setString(1, user.getName()); //id를 넣음.
        pstmt.setString(2, user.getRole()); //pw를 넣음.
        pstmt.setString(3, user.getId()); //name를 넣음.

        // executeUpdate : int값 반환 -> 몇 개의 행에 쿼리문이 영향을 끼쳤는지 반환
        int row = pstmt.executeUpdate(); // -> DB에 쿼리 요청 보냄
    ...
    public int delete(String id) throws SQLException {
        ...
        //DB 에 쿼리 요청 -> id가 pk일 때 정상동작하면 row = 1
